...
class RetrievalService:
    ...
    async def retrieve_context(self, query: str, top_k: int = 5, min_score: float = 0.3) -> List[RetrievedContext]:
        ...
        try:
            ...
            # Use the existing search method from VectorStorage with original query
            ...
            for result in search_results:
                ...
                try:
                    ...
                    # If score can't be converted to float, default to 0
                    numeric_score = 0.0
                if numeric_score >= min_score:
                    filtered_results.append(result)

            # If no results found with minimum score threshold, try a broader search
            if not filtered_results and min_score > 0.0:
                logger.info(f"No results found with min_score {min_score}, trying with lower threshold")
                search_results = await self.vector_storage.search(query, limit=top_k * 2)  # Search more results

                # Try with a much lower threshold
                lower_min_score = min(min_score / 2, 0.1)  # Use at most 0.1
                filtered_results = []
                for result in search_results:
                    score = result.get('score', 0)
                    try:
                        ...
                        numeric_score = 0.0
                    if numeric_score >= lower_min_score:
                        filtered_results.append(result)

                # If still no results, try with 0.0 threshold
                if not filtered_results:
                    logger.info("No results found, trying with 0.0 threshold")
                    search_results = await self.vector_storage.search(query, limit=top_k * 3)  # Even more results
                    filtered_results = []
                    for result in search_results:
                        score = result.get('score', 0)
                        try:
                            ...
                            numeric_score = 0.0
                        if numeric_score >= 0.0:  # Accept any results
                            filtered_results.append(result)
